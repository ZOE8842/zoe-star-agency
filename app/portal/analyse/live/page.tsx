...
import { LiveReportForm } from "./LiveReportForm";
...
export default async function LiveAnalyseHub() {
  const { supabase, profile } = await getAuthedProfile();

  const { data: rows } = await supabase
    .from("live_performance_reports")
    .select("id, period_label, status, created_at, manual_note")
    ...
    <>
      ...
      <main className="container-luxe relative z-10 py-12 md:py-16 max-w-2xl">
        ...
        <h1 className="font-display italic text-cream text-4xl md:text-5xl leading-[1.05] tracking-[-0.02em] mb-4">
          Wo stehst du <span className="text-champagne">im LIVE?</span>
        </h1>
        <p className="text-cream/60 text-base md:text-lg leading-relaxed mb-10 max-w-xl">
          Wir analysieren deine LIVE-Daten: Zuschauer-Schnitt, gueltige
          Tage, Watchtime, Battle-Auswertung, Match-Verhalten,
          Community-Staerke und Wachstums-Trends. Ergebnis: konkreter
          Wochen-Plan.
        </p>

        <LiveReportForm hasOpenReport={open.length > 0} />
