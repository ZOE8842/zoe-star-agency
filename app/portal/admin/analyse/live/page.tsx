...
export default async function AdminLiveReportsQueue() {
  const { supabase, profile } = await requireAdmin();

  const { data: rows } = await supabase
    .from("live_performance_reports")
    .select("id, profile_id, period_label, period_start, period_end, status, manual_note, created_at, cost_usd, ai_provider, ai_model")
    ...
    <>
      ...
      <main className="container-luxe py-12 md:py-16">
        ...
        <h1 className="font-display italic text-cream text-4xl md:text-5xl leading-[1.05] tracking-[-0.02em] mb-4">
          LIVE Performance <span className="text-champagne">Queue.</span>
        </h1>
        <p className="text-cream/60 text-base md:text-lg leading-relaxed mb-12 max-w-2xl">
          Web-Modul-Version des frueheren ZOE-App /zoecheck. KPI-Reports
          mit Wochen-Plan, Battle-Auswertung und Tageszeit-Vorschlaegen.
        </p>
        ...
        {enriched.length === 0 && (
          <p className="text-cream/45 italic font-display text-lg">Noch keine Reports.</p>
        )}
            ...
            <li key={r.id} className="border border-champagne/15 p-4 md:p-5">
              ...
              <p className="text-cream/55 text-[11px] uppercase tracking-[0.22em] mb-2">
                ...
                {r.period_start && r.period_end && (
                  <> · {new Date(r.period_start).toLocaleDateString("de-DE")} – {new Date(r.period_end).toLocaleDateString("de-DE")}</>
                )}
              </p>
              {r.manual_note && (
