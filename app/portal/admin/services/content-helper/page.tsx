...
export default async function AdminContentHelper() {
  const { supabase, profile } = await requireAdmin();

  const { data: rows } = await supabase
    .from("content_reviews")
    .select("id, profile_id, kind, source_url, status, manual_note, created_at, cost_usd, ai_provider")
  ...
  const enriched = (rows ?? []).map((r) => ({
    ...
    tiktok: map.get(r.profile_id)?.tiktok_username ?? null,
  }));
    ...
    <>
      ...
      <main className="container-luxe py-12 md:py-16">
        ...
        <h1 className="font-display italic text-cream text-4xl md:text-5xl leading-[1.05] tracking-[-0.02em] mb-4">
          Content Helfer <span className="text-champagne">Queue.</span>
        </h1>
        <p className="text-cream/60 text-base md:text-lg leading-relaxed mb-12 max-w-2xl">
          Eingereichte Videos / Bilder / Links / Profil-Checks der Creator.
          Cost-Logging + Status-Tracking.
        </p>
            ...
            <li key={r.id} className="border border-champagne/15 p-4 md:p-5">
              <div className="flex items-baseline justify-between gap-3 mb-2 flex-wrap">
                <p className="font-display italic text-cream text-lg md:text-xl">
                  {r.creator_name || r.tiktok || "—"}{" "}
                  {r.tiktok && <span className="text-cream/45 text-sm">@{r.tiktok}</span>}
                </p>
                <span className={`shrink-0 px-2.5 py-0.5 text-[10px] uppercase tracking-[0.25em] ${STATUS_TONE[r.status] ?? ""}`}>
                  {STATUS_LABEL[r.status] ?? r.status}
                </span>
              </div>
              <p className="text-cream/55 text-[11px] uppercase tracking-[0.22em] mb-2">{r.kind}</p>
              {r.source_url && (
                <p className="text-cream/65 text-xs break-all mb-2">{r.source_url}</p>
              )}
              {r.manual_note && (
